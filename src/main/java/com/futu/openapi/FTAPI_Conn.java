...
public class FTAPI_Conn implements AutoCloseable, ConnHandler {
   ...
   * @return 数据包序列号
   */
  protected Future sendProto(int protoID, GeneratedMessageV3 req) {
    synchronized (this) {
      FTAPI.netEventListener.onBeginSend(this, protoID);
      if (connStatus != ConnStatus.CONNECTED && connStatus != ConnStatus.READY) {
        SettableFuture future = SettableFuture.create();
        future.setException(new IllegalStateException("Wrong connStatus " + connStatus));
        return future;
      }
      ...
      sentData.future = SettableFuture.create();
  ...
  private void handleReplyPacket(ReqReplyType replyType, ProtoHeader header, byte[] body, boolean isRemoveFromSent) {
    ...
    if (header.nProtoID == ProtoID.INIT_CONNECT)
      ...
      onReply(replyType, sentData.future, header, body);
  }

  private void sendInitConnect() {
    ...
    sendProto(ProtoID.INIT_CONNECT, req);
  }
  ...
  private void sendKeepAlive() {
    ...
    sendProto(ProtoID.KEEP_ALIVE, req);
  }
